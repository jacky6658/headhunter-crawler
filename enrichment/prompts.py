...
2a. JOB_MATCH_SYSTEM_PROMPT: 職缺 context + 評分規則 (v4 拆分版 system)
2b. JOB_MATCH_USER_PROMPT: 候選人資料 (v4 拆分版 user)
2c. JOB_MATCH_PROMPT: 綜合版（向下相容）
...
# ============================================================
PROFILE_ANALYSIS_PROMPT = """你是專業獵頭顧問 AI。請深度搜尋並分析以下候選人的完整專業背景。

LinkedIn URL: {url}

**搜尋策略（非常重要！）**：
1. 先從 URL 提取人名（/in/ 之後的部分），用「姓名 + LinkedIn」搜尋
2. 再搜尋「姓名 + 公司名 + 職稱」獲取更多資訊
3. 搜尋「姓名 + GitHub」、「姓名 + 個人網站」、「姓名 + 技術演講」等
4. 如果找到 GitHub、個人部落格、公司官網，請深入閱讀這些頁面
5. 綜合所有來源的資訊，組合出最完整的職業經歷

**⚠️ 最重要的要求**：
- work_history 必須列出此人的【所有】工作經歷，不是只有目前的工作
- 每一段工作經歷都要有公司名、職稱、起訖時間
- education_details 必須列出所有學歷（大學、碩士、博士等）
- 寧可多列幾條、也不要只列一條就結束
...
注意事項：
- work_history 請按時間倒序排列，從最近的工作開始，列出所有找到的工作經歷（通常一個人有 3-8 段）
- years_experience 請根據最早的工作經歷到現在推算總年數（整數）
- stability_indicators 中 avg_tenure_months 請根據各段工作時長計算平均月數
- job_changes 為總共換過幾份工作
- recent_gap_months 為最近一次工作間隔月數（0 表示目前在職無間隔）
- skills 請盡量完整列出，包含：頁面上明確展示的、從工作經歷推斷的、從公司技術棧推斷的
- 如果完全無法取得資訊，仍請回傳 JSON 結構，填空值即可
- ⚠️ 必須回傳完整 JSON，不要回覆 "我無法存取" 之類的訊息
- ⚠️ work_history 只列一條是不合格的，請盡力搜尋完整經歷"""

# ============================================================
# Prompt 1b: 用姓名搜尋候選人背景 — 無 LinkedIn URL 時的替代方案
# ============================================================
NAME_SEARCH_PROMPT = """你是專業獵頭顧問 AI。請搜尋以下候選人的專業背景資訊。
...
**重要指示**：
1. 使用候選人的姓名、公司、職稱進行網路搜尋
2. 搜尋 LinkedIn、GitHub、個人網站、公司官網、技術部落格、會議演講等公開資訊
3. 如果有 GitHub URL，請查看其 profile 和 repositories 瞭解技術背景
4. 綜合所有找到的資訊，盡可能完整地填寫以下欄位
5. 即使資訊有限，也請根據可用資訊進行合理推斷
...
注意事項：
- 即使只找到 GitHub 資訊，也可以從 repositories、contributions、bio 推斷技術技能
- 從公司名稱和職稱推斷可能的產業和工作內容
- years_experience 根據找到的工作經歷推算，找不到則根據職稱資深度估計
- ⚠️ 必須回傳 JSON，不要回覆 "我無法找到" 之類的訊息"""
